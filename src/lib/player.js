...
class Player {
	...
	action_chatsend(data) {
		...
		if (action.data.charAt(0) == '/') {
			commands.execute(this, action.data);
		...
		for (var w = 0; w <= viewDistance; w++) {
			for (var x = 0 - w; x <= 0 + w; x++) {
				for (var z = 0 - w; z <= 0 + w; z++) {
					...
					if (worldManager.get(players[id].world).chunks[tempid] != undefined)
						worldManager.get(players[id].world).chunks[tempid].keepAlive();
